...
| Shop weapons and armour stayed a token +1 forever, regardless of depth | **fixed** - reported directly: "You find better things in the dungeon and you get showered with currency at every battle... Shops should contain extra buffed items at high prices but anytime you go in a store the items are super underpowered." Checked against what the same file already does for dungeon loot: `stock_for()`'s weaponsmith and armourer blocks rolled `enchant=1 if rng.random() < 0.25 (or 0.20) else 0` - a flat coin toss for a bare +1, the same at floor 1 as floor 30 - while `generate_item()` for anything found underground already calls the depth-scaling `roll_enchantment()` (weapons) and `roll_tier()` (armour, per the RPGClassics shrine's named +5/+10/+20 steps) built earlier this pass. A shelf that never got any better than a single dungeon floor's worst drop is exactly "super underpowered" against loot that keeps climbing, and it explains why gold felt like a nuisance: there was nothing worth spending a deep purse on. Both shop blocks now call the same depth-scaling rollers dungeon loot uses; a cursed roll is treated as a miss (enchant 0) rather than stocked, since a trader wouldn't knowingly sell a cursed blade. Verified directly: at depth 8 the weaponsmith's shelf now turns up a Runestaff +2 and a Broadsword +1 instead of nothing but +0s and +1s regardless of how deep the party had been. |
| Levelling up never taught a spell of its own | **fixed** - asked directly: "isn't there another way to learn spells besides tomes?" First answer (tomes only) was wrong, and said so once corrected: the manual's leveling section already lists "spells" among what a level increase adds to, right alongside hit points, mana and fighting skills, which was missed on the first pass through it. The follow-up quoted the Wikibooks CotW walkthrough directly: "The player character automatically gains a spell with each level-up, and can permanently gain another using the corresponding book (found or purchased), until he learns all the spells that he can learn." New `World.grant_level_spell()` runs on every level gained in `kill()`'s xp-award loop: it picks one random spell the character does not already know and is high-enough level and Intelligence for, the same gate `can_learn()` already applies to reading a tome, and hands it over for free - on top of tomes, never instead of them, so a spell learned from a book is not "spent" against the level-up grant and vice versa. Which spell among several eligible ones is granted is our own placement; the source does not say. The other half of that same walkthrough passage - "a player will instantly recognize a book for a spell they already know, [so] reading an unidentified spellbook will always guarantee you learn a new spell" - needed no change: every tome in Stormhold, dungeon or shop, is built by `spells.make_tome()`, which always sets `known = True`, so an unidentified tome is not a state that exists here to begin with; you already see what a book is before deciding to read it. |
| The first fight could cost several deaths in a row | **fixed** - reported directly: "Almost impossible not to spend several deaths on the first fight." Simulated directly over the real combat code (not a description of it): a fresh level-1 character with the starting dagger and no armour, meeting the largest cave rat pack the old numbers allowed (`pack=(2, 5)`, `dmg=(1, 3)`), died 5.6 times on average in a single encounter with the full pack of five - dying here means waking at the temple with the pack dropped on the floor and some xp and gold gone, not the run ending, but that number is exactly what "almost impossible not to spend several deaths" predicts, and cave_rat is by far the most frequent thing a new character meets (`freq=10`, `min_d=1`). Neither number is sourced - the bestiary's "when found in numbers can be much more deadly" gives no figures for pack size or damage - so both were ours to set, and they were set too high for the very first thing a new character fights. Halved the pack to `(1, 3)` and the top of the damage roll to 2; the same simulation's worst case (three rats at once) now averages under half a death, and one or two rats - the common case - average close to none. |
