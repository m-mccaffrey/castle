...
| A big magical bonus reads as visibly stronger, not just a bigger number | **fixed** - flagged directly: "Vote [note] has tiers, enchanted, strongly, very strongly enchanted." Checked against the actual `CASTLE1.EXE` string table (`strings -a`, not memory): there is no `Strongly Enchanted`/`Very Strongly Enchanted` string anywhere, so the item *name* stays exactly Normal/Enchanted/Cursed as above - that part of the report doesn't match what the binary ships. But the vocabulary is real, just attached to the property sentence instead: `"%i, makes the character strongly resistant %i, %i"` sits right next to the plain `"...makes the character resistant %i, %i"` template, and a separate `"greatly "`/`"very "` pair sit next to the stat-increase template. `Item.describe()` now opens a property sentence with `"strongly "` or `"greatly "` once the bonus is big enough - a +2 blade still reads plain, a +5 one reads `"greatly adds +5 to your chance to hit and to damage"`. The exact original cutoffs aren't recoverable from the string table alone, so the amount-to-adverb bands (3+, 5+) are our own placement of the game's real wording, flagged as such in `items.py`, not a sourced threshold. |
