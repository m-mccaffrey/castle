...
| Dropping a worn item refused, "no room", even though the drop was never going through the pack | **fixed** - reported directly: "Can't put things on floor if no room in pack." `_act_drop()` on a worn item goes through `unequip()` first, and `unequip()` checks pack room before letting anything come off, cursed items aside - the right check for the *actual* Take It Off verb, where the item really does stay in the pack afterward, but wrong here: `_act_drop()` immediately pulls it straight back out to put it on the floor, so a full pack was refusing room it was never going to occupy for longer than a heartbeat. `unequip()` takes a `skip_room_check` now, and `_act_drop()` passes it. |
