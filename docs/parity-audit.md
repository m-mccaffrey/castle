...
| A heavy enough purse could freeze a character permanently, with no way to shed the weight | **fixed** - asked directly: "Are you sure copper is supposed to weigh? It can't be dropped, so there's a condition where one could freeze themselves out of the game by carrying too much copper." Checked against source before changing anything: coin weight itself is faithful - the commit that added it quotes the manual directly, "Every coin weighs a gram, so a thousand of them weigh a kilo" - and past the game's own hard carry limit, movement genuinely stops (`action_cost(moving=True)` returns `None`). The gap was real, though, and specific to this port: the manual's money page implies coin can be carried as an ordinary pack object, not only as the abstract purse total ("[Copper] doesn't include any money you have in your pack" only makes sense if it can), but nothing in `_act_pickup`/`_act_take` ever created that form - a coin pile picked up always deposits straight into the purse, with no way back out, so an overloaded purse had no escape at all: not "Drop" (nothing to select), not the bank or a sale (both need a shop the character may not be near, and getting there is exactly the movement the weight has already stopped). New `drop_coins` action puts an exact, chosen amount of copper back on the ground as a real coin-pile item, the same kind a monster or the floor already drops - available from a new Drop Coin button in the Person panel, prompting for an amount the same way Name Object already prompts for text. Deliberately costed like every other Drop (`action_cost` without `moving=True`), so it works precisely in the one situation it exists for: while genuinely stuck. |
